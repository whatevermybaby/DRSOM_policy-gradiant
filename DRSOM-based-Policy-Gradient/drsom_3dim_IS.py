...
class DRSOMF(torch.optim.Optimizer):
  ...
  def __init__(
    ...
    for p in self._params:
      ...
      self.state[p]['momentum_for_g']=torch.zeros_like(p.data, requires_grad=True)
      self.state[p]['old_p']=torch.zeros_like(p.data, requires_grad=True)
    #
  ...
  def _clear_momentum(self):
    ...
    for p in self._params:
      ...
      self.state[p]['momentum_for_g'].zero_()
  
  def _apply_step(self, flat_new_p,flat_p, flat_d,flat_g):
    with torch.no_grad():
      offset = 0
      for p in self._params:
        numel = p.numel()
        # view as to avoid deprecated pointwise semantics
        p.copy_(flat_new_p[offset:offset + numel].view_as(p))
        self.state[p]['momentum'].copy_(flat_d[offset:offset + numel].view_as(p))
        self.state[p]['momentum_for_g'].copy_(flat_g[offset:offset + numel].view_as(p))
        self.state[p]['old_p'].copy_(flat_p[offset:offset + numel].view_as(p))
        offset += numel
      assert offset == self._numel()
  def _apply_back_to_old(self,flat_p):
    with torch.no_grad():
      offset = 0
      for p in self._params:
        numel = p.numel()
  ...
  def _directional_evaluate(self, closure, flat_new_p,flat_p,flat_d,flat_g):
    self._apply_step(flat_new_p,flat_p, flat_d,flat_g)
    # evaluation
  ...
  def update_trust_region(self, flat_p, flat_g, flat_d,flat_v,g_norm, d_norm,v_norm,old_p,old_g):
    with torch.enable_grad():
      ...
      if self.iter % self.freq == 0:
        ...
        if old_p !=0:
          ...
          gv, self._params,
          # create_graph=True,
          retain_graph=True, ), target='self')
        #   Hv = self._gather_flat_grad(torch.autograd.grad(
        #   (flat_g ** 2).sum() / 2, old_p,
        #   # create_graph=True,
        #   retain_graph=True,
        # ), target='self')
        else:
  ...
  def step(self, closure=None):
    ...
    if self.iter == 0 :
      ...
      old_flat_g=flat_g
      flat_v=flat_g-old_flat_g
      old_p=0
    else:
      # first calculate the gradient of last parameters

      tem_cl_new_p = self._clone_param()
      old_p = copy.deepcopy(self._apply_back_to_old(self._gather_flat_grad([self.state[p]['old_p'] for p in self._params])))
      # print("原参数",old_p)

      old_loss = closure()
      
      old_flat_g = parameters_to_vector([p.grad for p in self._params])
      # self._params.grad.zero_()
      # print("我的旧g",old_flat_g)

    # change the params to current param,calculate the current gradient
      self._set_param(tem_cl_new_p)
      loss=closure()
      if old_p == self._params:
        print('这咋一样呢？')
      #print("我的旧时光那",old_flat_g)
      # print("原原参数",self._params)
    ...
    self.update_trust_region(flat_p, flat_g, flat_d,flat_v,g_norm,d_norm,v_norm,old_p,old_flat_g)
    # accept or not?
    acc_step = False
    ...
    while iter_adj < self._max_iter_adj:
      ...
      # accept or not？
      loss_est = self._directional_evaluate(closure, flat_new_p, flat_p,flat_new_d,flat_g)
